...
# # # Step 8: Bayesian optimization bounds
# # bounds = {stock: (0,1) for stock in stocks}

# # # Step 9: running it
# # optimizer = BayesianOptimization(f=optimize_portfolio, pbounds=bounds)
# # optimizer.maximize(init_points=5, n_iter=20)

# # # Step 10: get optimized weights
# # optimized_weights = np.array(list(optimizer.max['params'].values()))

# # # Step 11: run the optimizaiton with these weights
# # opt_results = minimize(portfolio_variance, weights_guess, args=(returns,), method='SLSQP', constraints=constraints)

# # # Step 12: Extract the optimized weights and calculate other portfolio statistics
# # optimized_weights_final = opt_results.x
# # portfolio_return = np.sum(returns.mean() * optimized_weights_final)
# # portfolio_volatility = np.sqrt(np.dot(optimized_weights_final.T, np.dot(returns.cov(), optimized_weights_final)))

# # # Step 13: Print the optimized weights and portfolio statistics
# # print("Optimized Weights (Bayes):")
# # for stock, weight in zip(stocks, optimized_weights):
# #     print(stock + ":", weight)

# # print("Optimized Weights (Final):")
# # for stock, weight in zip(stocks, optimized_weights_final):
# #     print(stock + ":", weight)
...
optimizer = BayesianOptimization(f=objective, pbounds=dict(zip(stocks, [(-1, 1)] * len(stocks))), verbose=2)
optimizer.maximize(init_points=10, n_iter=50)

# Extract the optimized weights
optimized_weights = np.array([optimizer.max['params'][stock] for stock in stocks])
